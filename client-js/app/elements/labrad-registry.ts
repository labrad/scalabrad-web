...
export class LabradRegistry extends polymer.Base {
  ...
  _computeSelectedType(selectedIdx: number): string {
    var ofs = this.path.length > 0 ? 1 : 0;
    if (this.dirs && selectedIdx < this.dirs.length + ofs) {
      return 'dir';
    } else {
      return 'key';
    }
  ...
  _computeSelectedItem(selectedIdx: number): string {
    var ofs = this.path.length > 0 ? 1 : 0;
    if (this.dirs) {
      return this.dirs.map(it => it.name).concat(this.keys.map(it => it.name))[selectedIdx - ofs];
    }
  }
